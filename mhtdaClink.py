...
#lib = CDLL("../scaleDA4MHT/fastmurty/mhtda.so", RTLD_GLOBAL)
#sparse = False
lib = CDLL("../scaleDA4MHT/sparsemodified/mhtda.so", RTLD_GLOBAL)
sparse = True
...
def sparsifyByRow(c, nvalsperrow):
    ...
    mod 9/14/19 adding in zero-column case
    """
    m,n = c.shape
    nvalsperrow = min(n, nvalsperrow)
    nvals = m*nvalsperrow
    if nvalsperrow == 0:
        cp = np.zeros(m+1, dtype=np.int32)
    else:
    ...
    for i, crow in enumerate(c):
        colsinorder = np.argsort(crow)[:nvalsperrow]
#        if nvalsperrow < n:
